...
function lbHeartsHTML(hp, maxHp = 40) {
  ...
  const phaseHp = inUpperPhase ? hp - half : hp; // 現在のフェーズでの残りHP(0〜20)
  const colorClass = inUpperPhase ? 'lb-heart-blue' : '';
  const totalHearts = 10;
  const perHeart = half / totalHearts; // 2
  let html = '';
  for (let i = 0; i < totalHearts; i++) {
    const val = phaseHp - i * perHeart;
    const state = val >= perHeart ? 'full' : val >= perHeart / 2 ? 'half' : 'empty';
    html += `<span class="lb-heart ${state} ${colorClass}"></span>`;
  }
